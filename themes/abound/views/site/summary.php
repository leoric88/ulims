...
echo "test";
/* Statistics by Barangay : Start */
/*	$barangays = Barangay::model()->findAll();
	
	$count=1;
	$totalIndividualsBarangay = 0;
	$totalBarangay = 0;
	$statBarangay= array();
	foreach($barangays as $barangay){
		$individualsPerBarangay = 0;
		$households = Household::model()->findAll('barangay_id=:barangay_id', 
					  array(':barangay_id'=>$barangay->id));
					  
		foreach($households as $household){
		  	set_time_limit(760);
			$individualsPerBarangay += $household->noOfFamilyMembers;
		}
		
		$familiesCount = Household::model()->count('barangay_id=:barangay_id', 
					  array(':barangay_id'=>$barangay->id));
					 
		$raw = array(
			'id'=>$count,
			'site'=>$barangay->name,
			'individual'=>$individualsPerBarangay,
			'families'=>$familiesCount
		);
		if($familiesCount != 0){
			array_push($statBarangay, $raw);
			$count += 1;
			$totalIndividualsBarangay = $totalIndividualsBarangay + $individualsPerBarangay;
			$totalBarangay = $totalBarangay + $familiesCount;
		}
	}
	...
	$raw = array(
			'id'=>'',
			'site'=>TOTAL,
			'individual'=>$totalIndividualsBarangay,
			'families'=>$totalBarangay
		);
	array_push($statBarangay, $raw);
		
	$gridDataProviderBarangay = new CArrayDataProvider($statBarangay, array('pagination'=>array(
        'pageSize'=>100,
    )));
*/
/* Statistics by Barangay : End */

/* Statistics by Evacuation Center : Start */
/*
	$evacuations = Evacuation::model()->findAll();
	$count=1;
	$totalIndividualEvac = 0;
	$totalEvacCenter = 0;
	$statEvacCenter = array();
	foreach($evacuations as $evac){
		$individualsPerEvac = 0;
		$households = Household::model()->findAll('evacuation_id=:evacuation_id', 
					  array(':evacuation_id'=>$evac->id));
					  
		foreach($households as $household){
		  	set_time_limit(760);
			$individualsPerEvac += $household->noOfFamilyMembers;
		}
		
		$familiesCount = Household::model()->count('evacuation_id=:evacuation_id', 
					  array(':evacuation_id'=>$evac->id));

		$raw = array(
			'id'=>$count,
			'site'=>$evac->site,
			'individual'=>$individualsPerEvac,
			'families'=>$familiesCount
		);
		
		array_push($statEvacCenter, $raw);
		$count += 1;
		$totalIndividualEvac = $totalIndividualEvac + $individualsPerEvac;
		$totalEvacCenter = $totalEvacCenter + $familiesCount;
	}
	
	$raw = array(
			'id'=>'',
			'site'=>TOTAL,
			'individual'=>$totalIndividualEvac,
			'families'=>$totalEvacCenter
		);
	array_push($statEvacCenter, $raw);
		
	$gridDataProviderEvacuation = new CArrayDataProvider($statEvacCenter, array('pagination'=>array(
        'pageSize'=>100,
    )));
*/
/* Statistics by Evacuation Center : End */
?>
